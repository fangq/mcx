////////////////////////////////////////////////////////////////////////////////
//
//  Monte Carlo eXtreme (MCX)  - GPU accelerated 3D Monte Carlo transport
//  simulation
...
// Parameters for workload partitioning
std::string wk_config = ("");
enum WK_Stat { COMPUTE, USEPREV };  // specify wk mode
WK_Stat wk_stat = COMPUTE;          // compute wk as default
char buffer[100];                   // save the info
float *wk_a;                        // prediction model paramter a, ms
float *wk_b;                        // prediction model paramter b, ms
float *sampling1;                   // sampling point at 1M photons for device
float *sampling2;                   // sampling point at 2M photons for device
float *timeDev;                     // predicted time for each device
float *freqDev;                     // inversion of time for each device
float *probDev;                     // workload portion for each device
int workdev = 0;  // working devices
FILE *fo;
...
__device__ inline void clearpath(float *p, int maxmediatype) {
  uint i;
  for (i = 0; i < maxmediatype; i++) p[i] = 0.f;
}
    ...
    for (z = gcfg->cp0.z; z <= gcfg->cp1.z; z++)
      for (y = gcfg->cp0.y; y <= gcfg->cp1.y; y++)
        for (x = gcfg->cp0.x; x <= gcfg->cp1.x; x++) {
          atomicadd(
              data + z * gcfg->dimlen.y + y * gcfg->dimlen.x + x,
              cache[(z - gcfg->cp0.z) * gcfg->cachebox.y +
                    (y - gcfg->cp0.y) * gcfg->cachebox.x + (x - gcfg->cp0.x)]);
        }
  }
}
#endif
...
__device__ inline uint finddetector(MCXpos *p0) {
  ...
  for (i = 0; i < gcfg->detnum; i++) {
    if ((gdetpos[i].x - p0->x) * (gdetpos[i].x - p0->x) +
            ...
            (gdetpos[i].z - p0->z) * (gdetpos[i].z - p0->z) <
        gdetpos[i].w * gdetpos[i].w) {
      return i + 1;
    }
  }
  return 0;
...
__device__ inline void savedetphoton(float n_det[], uint *detectedphoton,
                                     float nscat, float *ppath, MCXpos *p0,
  ...
  if (detid) {
    ...
    if (baseaddr < gcfg->maxdetphoton) {
      ...
      for (i = 0; i < gcfg->maxmedia; i++)
        n_det[baseaddr + i] =
            ppath[i];  // save partial pathlength to the memory
    }
  }
}
#endif
...
__device__ inline float hitgrid(float3 *p0, float3 *v, float3 *htime, int *id) {
  ...
  // p0 is inside, p is outside, move to the 1st intersection pt, now in the air
  // side, to be corrected in the else block
  ...
  (*id == 0)
      ? (htime->x = nextafterf(__float2int_rn(htime->x),
                               htime->x + (v->x > 0.f) - 0.5f))
      : ((*id == 1) ? (htime->y = nextafterf(__float2int_rn(htime->y),
                                             htime->y + (v->y > 0.f) - 0.5f))
                    : (htime->z = nextafterf(__float2int_rn(htime->z),
                                             htime->z + (v->z > 0.f) - 0.5f)));
...
__device__ inline void transmit(MCXdir *v, float n1, float n2, int flipdir) {
  ...
  (flipdir == 0)
      ? (v->x = sqrtf(1.f - v->y * v->y - v->z * v->z) *
                ((v->x > 0.f) - (v->x < 0.f)))
      : ((flipdir == 1) ? (v->y = sqrtf(1.f - v->x * v->x - v->z * v->z) *
                                  ((v->y > 0.f) - (v->y < 0.f)))
...
__device__ inline int skipvoid(MCXpos *p, MCXdir *v, MCXtime *f,
                               uchar media[]) {
  int count = 1, idx1d;
  while (1) {
    if (p->x >= 0.f && p->y >= 0.f && p->z >= 0.f && p->x < gcfg->maxidx.x &&
      ...
      if (media[idx1d]) {  // if inside
        ...
        count = 0;
        while (!(p->x >= 0.f && p->y >= 0.f && p->z >= 0.f &&
                 p->x < gcfg->maxidx.x && p->y < gcfg->maxidx.y &&
                 p->z < gcfg->maxidx.z) ||
               !media[idx1d]) {  // at most 3 times
          f->t += gcfg->minaccumtime *
                  hitgrid((float3 *)p, (float3 *)v, &htime, &flipdir);
          *((float4 *)(p)) = float4(htime.x, htime.y, htime.z, p->w);
          ...
          if (count++ > 3) {
            GPUDEBUG(
                ("fail to find entry point after 3 iterations, something is "
                 "wrong, abort!!"));
            break;
          }
        }
        ...
        if (gproperty[media[idx1d]].w != gproperty[0].w) {
          p->w *= 1.f - reflectcoeff(v, gproperty[0].w,
                                     gproperty[media[idx1d]].w, flipdir);
    ...
    *((float4 *)(p)) = float4(p->x + v->x, p->y + v->y, p->z + v->z, p->w);
    ...
    f->t += gcfg->minaccumtime;
    if (count++ > gcfg->maxvoidstep) return -1;
  }
}
...
__device__ inline int launchnewphoton(
    MCXpos *p, MCXdir *v, MCXtime *f, Medium *prop, uint *idx1d, uchar *mediaid,
    float *w0, float *Lmove, uchar isdet, float ppath[], float energyloss[],
    float energylaunched[], float n_det[], uint *dpnum,
    RandType t[RAND_BUF_LEN], RandType tnew[RAND_BUF_LEN],
    RandType photonseed[RAND_BUF_LEN], uchar media[], float srcpattern[],
    int threadid, RandType rngseed[], RandType seeddata[]) {
  int launchattempt = 1;
  ...
  if (gcfg->seed == SEED_FROM_FILE) {
    int seedoffset = (threadid * gcfg->threadphoton +
                      min(threadid, gcfg->oddphotons - 1) + (int)f->ndone) *
                     RAND_BUF_LEN;
    for (int i = 0; i < RAND_BUF_LEN; i++) t[i] = rngseed[seedoffset + i];
  }
  do {
    *((float4 *)p) = gcfg->ps;
    *((float4 *)v) = gcfg->c0;
    ...
    *mediaid = gcfg->mediaidorig;
    if (gcfg->issaveseed) copystate(t, photonseed);

    if (gcfg->srctype == MCX_SRC_PENCIL) { /*source can be outside*/
                                           // do nothing
    } else if (gcfg->srctype == MCX_SRC_PLANAR ||
               gcfg->srctype == MCX_SRC_PATTERN ||
               gcfg->srctype ==
                   MCX_SRC_FOURIER) { /*a rectangular grid over a plane*/
      rand_need_more(t, tnew);
      RandType rx = rand_uniform01(t[0]);
      rand_need_more(t, tnew);
      RandType ry = rand_uniform01(t[0]);
      *((float4 *)p) =
          float4(p->x + rx * gcfg->srcparam1.x + ry * gcfg->srcparam2.x,
                 p->y + rx * gcfg->srcparam1.y + ry * gcfg->srcparam2.y,
                 p->z + rx * gcfg->srcparam1.z + ry * gcfg->srcparam2.z, p->w);
      if (gcfg->srctype == MCX_SRC_PATTERN)  // need to prevent rx/ry=1 here
        p->w = srcpattern[(int)(ry * JUST_BELOW_ONE * gcfg->srcparam2.w) *
                              (int)(gcfg->srcparam1.w) +
                          (int)(rx * JUST_BELOW_ONE * gcfg->srcparam1.w)];
      else if (gcfg->srctype == MCX_SRC_FOURIER) {
        p->w = (cosf((floorf(gcfg->srcparam1.w) * rx +
                      floorf(gcfg->srcparam2.w) * ry + gcfg->srcparam1.w -
                      floorf(gcfg->srcparam1.w)) *
                     TWO_PI) *
                    (1.f - gcfg->srcparam2.w + floorf(gcfg->srcparam2.w)) +
                1.f) *
      ...
      *idx1d = (int(floorf(p->z)) * gcfg->dimlen.y +
      ...
      rand_need_more(t, tnew);
    } else if (gcfg->srctype == MCX_SRC_FOURIERX ||
               gcfg->srctype == MCX_SRC_FOURIERX2D) {  // [v1x][v1y][v1z][|v2|];
                                                       // [kx][ky][phi0][M],
                                                       // unit(v0) x
                                                       // unit(v1)=unit(v2)
      rand_need_more(t, tnew);
      ...
      v2.w *= rsqrt(gcfg->srcparam1.x * gcfg->srcparam1.x +
                    gcfg->srcparam1.y * gcfg->srcparam1.y +
                    gcfg->srcparam1.z * gcfg->srcparam1.z);
      v2.x = v2.w *
             (gcfg->c0.y * gcfg->srcparam1.z - gcfg->c0.z * gcfg->srcparam1.y);
      v2.y = v2.w *
             (gcfg->c0.z * gcfg->srcparam1.x - gcfg->c0.x * gcfg->srcparam1.z);
      v2.z = v2.w *
             (gcfg->c0.x * gcfg->srcparam1.y - gcfg->c0.y * gcfg->srcparam1.x);
      *((float4 *)p) = float4(p->x + rx * gcfg->srcparam1.x + ry * v2.x,
                              p->y + rx * gcfg->srcparam1.y + ry * v2.y,
                              p->z + rx * gcfg->srcparam1.z + ry * v2.z, p->w);
      if (gcfg->srctype == MCX_SRC_FOURIERX2D)
        p->w =
            (sinf((gcfg->srcparam2.x * rx + gcfg->srcparam2.z) * TWO_PI) *
                 sinf((gcfg->srcparam2.y * ry + gcfg->srcparam2.w) * TWO_PI) +
             1.f) *
            0.5f;  // between 0 and 1
      else
        p->w = (cosf((gcfg->srcparam2.x * rx + gcfg->srcparam2.y * ry +
                      gcfg->srcparam2.z) *
                     TWO_PI) *
      ...
      *idx1d = (int(floorf(p->z)) * gcfg->dimlen.y +
      ...
      rand_need_more(t, tnew);
    } else if (gcfg->srctype == MCX_SRC_DISK ||
               gcfg->srctype == MCX_SRC_GAUSSIAN) {  // uniform disk
                                                     // distribution or
                                                     // Gaussian-beam
      ...
      *idx1d = (int(floorf(p->z)) * gcfg->dimlen.y +
    ...
    } else if (gcfg->srctype == MCX_SRC_CONE ||
               gcfg->srctype == MCX_SRC_ISOTROPIC ||
      ...
      if (gcfg->srctype ==
        ...
        do {
          rand_need_more(t, tnew);
          ang = (gcfg->srcparam1.y > 0) ? TWO_PI * rand_uniform01(t[0])
                                        : acosf(2.f * rand_uniform01(t[0]) -
                                                1.f);  // sine distribution
        ...
        if (gcfg->srctype == MCX_SRC_ISOTROPIC)  // uniform sphere
          ...
          ang = ONE_PI *
                rand_uniform01(
                    t[0]);  // uniform distribution in zenith angle, arcsine
      }
      ...
      sincosf(ang, &sphi, &cphi);
      rand_need_more(t, tnew);
      ang = sqrtf(-2.f * logf(rand_uniform01(t[0]))) * (1.f - 2.f * t[1]) *
            gcfg->srcparam1.x;
      sincosf(ang, &stheta, &ctheta);
      rotatevector(v, stheta, ctheta, sphi, cphi);
      ...
      *((float4 *)p) =
          float4(p->x + r * gcfg->srcparam1.x, p->y + r * gcfg->srcparam1.y,
                 p->z + r * gcfg->srcparam1.z, p->w);
      if (gcfg->srctype == MCX_SRC_LINE) {
        ...
        rand_need_more(t, tnew);
        p = sqrt(1.f - v->x * v->x - v->y * v->y) *
            (rand_uniform01(t[0]) > 0.5f ? 1.f : -1.f);
        *((float4 *)v) = float4(v->y * p - v->z * s, v->z * r - v->x * p,
                                v->x * s - v->y * r, v->nscat);
      }
    }
    if ((*mediaid & MED_MASK) == 0) {
      int idx = skipvoid(
          p, v, f,
          media); /*specular reflection of the bbx is taken care of here*/
      if (idx >= 0) {
        *idx1d = idx;
        *mediaid = media[*idx1d];
      }
    }
    if (launchattempt++ > gcfg->maxvoidstep) return -1;  // launch failed
  } while ((*mediaid & MED_MASK) == 0 || p->w <= gcfg->minenergy);
  f->ndone++;  // launch successfully
  *((float4 *)(prop)) =
      gproperty[*mediaid & MED_MASK];  // always use mediaid to read gproperty[]
...
kernel void mcx_test_rng(float field[], uint n_seed[]) {
  ...
  for (i = 0; i < len; i += RAND_BUF_LEN) {
    rand_need_more(t, tnew);
    for (j = 0; j < min(RAND_BUF_LEN, len - i); j++) field[i + j] = t[j];
  }
}
...
*/
kernel void mcx_main_loop(uchar media[], float field[], float genergy[],
                          uint n_seed[], float4 n_pos[], float4 n_dir[],
                          float4 n_len[], float n_det[], uint detectedphoton[],
                          float srcpattern[], float replayweight[],
                          float photontof[], RandType *seeddata) {
  int idx = blockDim.x * blockIdx.x + threadIdx.x;

  MCXpos p = {0.f, 0.f, 0.f,
              -1.f};  //{x,y,z}: coordinates in grid unit, w:packet weight
  MCXdir *v =
      (MCXdir *)(sharedmem + (threadIdx.x << 2));  //{x,y,z}: unitary direction
                                                   //vector in grid unit,
                                                   //nscat:total scat event
  ...
  RandType *t =
      (RandType *)(sharedmem + (blockDim.x << 2) + threadIdx.x * RAND_BUF_LEN);
  RandType tnew[RAND_BUF_LEN];  // ok withou initialization, will be assigned
                                // before use
  RandType photonseed[RAND_BUF_LEN];
  Medium
      prop;  // can become float2 if no reflection (mua/musp is in 1/grid unit)
  ...
  if (gcfg->skipradius2 > EPS)
    clearcache(cachebox, (gcfg->cp1.x - gcfg->cp0.x + 1) *
                             (gcfg->cp1.y - gcfg->cp0.y + 1) *
                             (gcfg->cp1.z - gcfg->cp0.z + 1));
#else
  ...
  if (launchnewphoton(&p, v, &f, &prop, &idx1d, &mediaid, &w0, &Lmove, 0, ppath,
                      &energyloss, &energylaunched, n_det, detectedphoton, t,
                      tnew, photonseed, media, srcpattern, idx,
                      (RandType *)n_seed, seeddata)) {
  ...
  /*
   using a while-loop to terminate a thread by np.will cause MT RNG to be 3.5x
   slower
   LL5 RNG will only be slightly slower than for-loop.with photon-move criterion

   we have switched to while-loop since v0.4.9, as LL5 was only minimally
   effected
  ...
  */
  ...
  while (f.ndone <= (gcfg->threadphoton + (idx < gcfg->oddphotons))) {
    ...
    if (f.pscat <= 0.f) {  // if this photon has finished his current jump, get
                           // next scat length & angles
      if (moves++ > gcfg->reseedlimit) {
        moves = 0;
        gpu_rng_reseed(
            t, tnew, n_seed, idx,
            (p.x + p.y + p.z + p.w) + f.ndone * (v->x + v->y + v->z));
      }
      f.pscat = rand_next_scatlen(
          t, tnew);  // random scattering probability, unit-less
      ...
      if (p.w < 1.f) {  // if this is not my first jump
        ...
        if (prop.g >
            EPS) {  // if prop.g is too small, the distribution of theta is bad
          tmp0 = (1.f - prop.g * prop.g) /
                 (1.f - prop.g + 2.f * prop.g * rand_next_zangle(t, tnew));
          tmp0 *= tmp0;
          tmp0 = (1.f + prop.g * prop.g - tmp0) / (2.f * prop.g);

          // when ran=1, CUDA gives me 1.000002 for tmp0 which produces nan
          // later
    ...
    *((float4 *)(&prop)) = gproperty[mediaid & MED_MASK];
    ...
    len = (gcfg->faststep) ? gcfg->minstep
                           : hitgrid((float3 *)&p, (float3 *)v, &htime,
                                     &flipdir);  // propagate the photon to the
                                                 // first intersection to the
                                                 // grid
    slen = len * prop.mus;  // unitless (minstep=grid, mus=1/grid)

    GPUDEBUG(("p=[%f %f %f] -> <%f %f %f>*%f -> hit=[%f %f %f] flip=%d\n", p.x,
              p.y, p.z, v->x, v->y, v->z, len, htime.x, htime.y, htime.z,
              flipdir));
    ...
    len = slen / prop.mus;
    *((float3 *)(&p)) =
        (gcfg->faststep || slen == f.pscat)
            ? float3(p.x + len * v->x, p.y + len * v->y, p.z + len * v->z)
            : float3(htime.x, htime.y, htime.z);
...
#ifdef SAVE_DETECTORS
    if (gcfg->savedet) ppath[(mediaid & MED_MASK) - 1] += len;  //(unit=grid)
#endif
    ...
    idx1d = (int(floorf(p.z)) * gcfg->dimlen.y +
             int(floorf(p.y)) * gcfg->dimlen.x + int(floorf(p.x)));
    GPUDEBUG(("idx1d [%d]->[%d]\n", idx1dold, idx1d));
    if (p.x < 0 || p.y < 0 || p.z < 0 || p.x >= gcfg->maxidx.x ||
        p.y >= gcfg->maxidx.y || p.z >= gcfg->maxidx.z) {
      mediaid = 0;
    } else {
    ...
    if (idx1d != idx1dold && idx1dold > 0 && mediaidold) {
      ...
      if (gcfg->save2pt && f.t >= gcfg->twin0 && f.t < gcfg->twin1) {
...
#ifdef TEST_RACING
        // enable TEST_RACING to determine how many missing accumulations due to
        // race
        if ((p.x - gcfg->ps.x) * (p.x - gcfg->ps.x) +
                (p.y - gcfg->ps.y) * (p.y - gcfg->ps.y) +
                (p.z - gcfg->ps.z) * (p.z - gcfg->ps.z) >
            gcfg->skipradius2) {
        ...
        if (!gcfg->isatomic) {
#endif
          // set gcfg->skipradius2 to only start depositing energy when
          // dist^2>gcfg->skipradius2
          if (gcfg->skipradius2 > EPS) {
#ifdef USE_CACHEBOX
            if (p.x < gcfg->cp1.x + 1.f && p.x >= gcfg->cp0.x &&
              ...
              atomicadd(cachebox + (int(p.z - gcfg->cp0.z) * gcfg->cachebox.y +
                                    int(p.y - gcfg->cp0.y) * gcfg->cachebox.x +
                                    int(p.x - gcfg->cp0.x)),
                        weight);
#else
          if ((p.x - gcfg->ps.x) * (p.x - gcfg->ps.x) +
      ...
      w0 = p.w;
      Lmove = 0.f;
      // f.tnext+=gcfg->minaccumtime*prop.n; // fluence is a
      // temporal-integration, unit=s
    }

    // launch new photon when exceed time window or moving from non-zero voxel
    // to zero voxel without reflection

    if ((mediaid == 0 && (!gcfg->doreflect ||
                          (gcfg->doreflect && n1 == gproperty[mediaid].w))) ||
        f.t > gcfg->twin1) {
      GPUDEBUG(("direct relaunch at idx=[%d] mediaid=[%d], ref=[%d]\n", idx1d,
                mediaid, gcfg->doreflect));
      if (launchnewphoton(&p, v, &f, &prop, &idx1d, &mediaid, &w0, &Lmove,
                          (mediaidold & DET_MASK), ppath, &energyloss,
                          &energylaunched, n_det, detectedphoton, t, tnew,
                          photonseed, media, srcpattern, idx,
                          (RandType *)n_seed, seeddata))
        break;
      continue;
    }
    ...
    if (gcfg->doreflect && n1 != gproperty[mediaid].w) {
      ...
      *((float4 *)(&prop)) =
          gproperty[mediaid];  // optical property across the interface

      tmp0 = n1 * n1;
      tmp1 = prop.n * prop.n;
      cphi = fabs((flipdir == 0) ? v->x
                                 : (flipdir == 1 ? v->y : v->z));  // cos(si)
      ...
      if (Rtotal < 1.f &&
          rand_next_reflect(t, tnew) > Rtotal) {  // do transmission
        ...
        GPUDEBUG(("ref faceid=%d p=[%f %f %f] v_old=[%f %f %f]\n", flipdir, p.x,
                  p.y, p.z, v->x, v->y, v->z));
        (flipdir == 0) ? (v->x = -v->x)
                       : ((flipdir == 1) ? (v->y = -v->y) : (v->z = -v->z));
        (flipdir == 0)
            ? (p.x = nextafterf(__float2int_rn(p.x), p.x + (v->x > 0.f) - 0.5f))
            : ((flipdir == 1) ? (p.y = nextafterf(__float2int_rn(p.y),
                                                  p.y + (v->y > 0.f) - 0.5f))
                              : (p.z = nextafterf(__float2int_rn(p.z),
                                                  p.z + (v->z > 0.f) - 0.5f)));
...
Shared Memory:\t\t%u B\nRegisters:\t\t%u\nClock Speed:\t\t%.2f GHz\n",
    ...
    // wk : save device info
    if (cfg->wkpartition) {
      sprintf(buffer, "device %d: %s\n", dev, (*info)[dev].name);
      wk_config.append(buffer);
    }
  }
  if (cfg->isgpuinfo == 2 &&
  ...
  if (cfg->nblocksize > N - M) {
    mcx_error(-1,
              "block size can not be larger than 227 when using MT19937 RNG",
              __FILE__, __LINE__);
  }
#endif
...
void mcx_run_simulation(Config *cfg, GPUInfo *gpu) {
  ...
  MCXParam param = {
      cfg->steps, minstep, 0, 0, cfg->tend, R_C0 * cfg->unitinmm,
      cfg->issave2pt, cfg->isreflect, cfg->isrefint, cfg->issavedet,
      1.f / cfg->tstep, p0, c0, maxidx, uint3(0, 0, 0), cp0, cp1, uint2(0, 0),
      cfg->minenergy, cfg->sradius * cfg->sradius,
      minstep * R_C0 * cfg->unitinmm, cfg->srctype, cfg->srcparam1,
      cfg->srcparam2, cfg->voidtime, cfg->maxdetphoton, cfg->medianum - 1,
      cfg->detnum, 0, 0, cfg->reseedlimit,
      ABS(cfg->sradius +
          2.f)<EPS /*isatomic*/, cfg->maxvoidstep, cfg->issaveseed> 0,
      cfg->maxdetphoton * (cfg->medianum + 1), cfg->seed, cfg->outputtype, 0, 0,
      cfg->faststep};
  int detreclen = cfg->medianum + 1;
  if (param.isatomic) param.skipradius2 = 0.f;
  ...
  if (gpu[gpuid].maxgate == 0 && dimxyz > 0) {
    int needmem = dimxyz + cfg->nthread * sizeof(float4) * 4 +
                  sizeof(float) * cfg->maxdetphoton * (cfg->medianum + 1) +
                  10 * 1024 * 1024; /*keep 10M for other things*/
    gpu[gpuid].maxgate = (gpu[gpuid].globalmem - needmem) /
                         (cfg->dim.x * cfg->dim.y * cfg->dim.z);
    gpu[gpuid].maxgate =
        MIN((unsigned int)((cfg->tend - cfg->tstart) / cfg->tstep + 0.5),
            gpu[gpuid].maxgate);
  }
...
#pragma omp master
  {
    if (cfg->exportfield == NULL)
      cfg->exportfield =
          (float *)calloc(sizeof(float) * cfg->dim.x * cfg->dim.y * cfg->dim.z,
                          gpu[gpuid].maxgate * 2);
    if (cfg->exportdetected == NULL)
      cfg->exportdetected = (float *)malloc((cfg->medianum + 1) *
                                            cfg->maxdetphoton * sizeof(float));
...
#pragma omp barrier
  ...
  MCX_FPRINTF(cfg->flog,
              "autopilot mode: setting thread number to %d, block size to %d "
              "and time gates to %d\n",
  ...
  fullload = 0.f;
  for (i = 0; cfg->deviceid[i]; i++) fullload += cfg->workload[i];
...
#pragma omp master
  {
    if (totalgates > gpu[gpuid].maxgate && cfg->isnormalized) {
      MCX_FPRINTF(stderr,
          "WARNING: GPU memory can not hold all time gates, disabling "
          "normalization to allow multiple runs\n");
      cfg->isnormalized = 0;
    }

    // wk : save the kernel configuration for each gpu device
    if (cfg->wkpartition) {
      workdev = omp_get_num_threads();

      for (int i = 0; i < workdev; i++) {
        sprintf(buffer, "device %d: %d %d %d\n", i, gpu[i].autothread,
            gpu[i].autoblock, gpu[i].maxgate);
        wk_config.append(buffer);
      }

      // allocate a,b coefficients for each device
      probDev = (float *)malloc(sizeof(float) * workdev);
      wk_a = (float *)malloc(sizeof(float) * workdev);
      wk_b = (float *)malloc(sizeof(float) * workdev);
      sampling1 = (float *)malloc(sizeof(float) * workdev);
      sampling2 = (float *)malloc(sizeof(float) * workdev);
      timeDev = (float *)malloc(sizeof(float) * workdev);
      freqDev = (float *)malloc(sizeof(float) * workdev);

      // check whether there is a work_config file
      if (access(".work_config", F_OK) != -1) {
        std::ifstream file;
        std::string line_content;
        std::string str_in = ("");
        int line_count = 0;

        file.open(".work_config", std::ifstream::in);

        // read the top 4 lines
        if (file.is_open()) {
          while (std::getline(file, line_content)) {
            line_count++;
            str_in = str_in + line_content + "\n";
            if (line_count == 4) break;
          }
          file.close();
        } else {
          std::cout << "Unable to open work_config.\n";
          exit(1);
        }

        // compare with the wk_config string
        if (wk_config.compare(str_in) != 0) {
          // 1) they are not identical, update with current configuration
          fo = fopen(".work_config", "w");
          if (fo == NULL) {
            fprintf(stderr, "Fail to create .work_config file.\n");
            exit(1);
          }
          fprintf(fo, "%s", wk_config.c_str());
          fclose(fo);
        } else {
          // 2) they are identical, use previous setup
          wk_stat = USEPREV;
          std::cout << "Use previous setup" << std::endl;

          // read a, b for each device

          file.open(".work_config", std::ifstream::in);
          line_count = 0;
          int id = 0;  // the gpu device id
          int col = 0; // col 0 is a, col 1 is b
          if (file.is_open()) {
            while (std::getline(file, line_content)) {
              line_count++;
              // read line >=5 for a, b 
              if (line_count >= 5) {
                ...
                char *dup = strdup(line_content.c_str());
                char *token = strtok(dup, " ");
                col = 0;
                while (token != NULL) {
                  // printf("%f\n", atof(token));
                  // read a
                  if(col == 0) wk_a[id] = atof(token);
                  // read b
                  if(col == 1) wk_b[id] = atof(token);
                  // read next column 
                  col++;
                  token = strtok(NULL, " ");
                }
                free(dup);
                // read next device info
                id++;
              } // end of reading device info
            } // end of while()
          ...
          file.close();
          // end of updating wk when two setups are indentical
        } // end of comparison 

      } else {
        // If the file doesn't exist, save the current configuration
        fo = fopen(".work_config", "w");
        if (fo == NULL) {
          fprintf(stderr, "Fail to create .work_config file. file :%s, line: %d\n",
           __FILE__, __LINE__);
          exit(1);
        }
        fprintf(fo, "%s", wk_config.c_str());
        fclose(fo);
      }  // end of if-else checking
      // std::cout << wk_config << std::endl;
    }  // end of wk
  }
#pragma omp barrier
...
#pragma omp master
    {
      ...
      for (i = 0; i < RAND_SEED_LEN; i++) Pseed[i] = rand();
      mcx_cu_assess(cudaMalloc((void **)&gPseed, sizeof(uint) * RAND_SEED_LEN),
                    __FILE__, __LINE__);
      mcx_cu_assess(cudaMemcpy(gPseed, Pseed, sizeof(uint) * RAND_SEED_LEN,
                               cudaMemcpyHostToDevice),
                    __FILE__, __LINE__);
      mcx_cu_assess(cudaMalloc((void **)&gfield, sizeof(float) * fieldlen),
                    __FILE__, __LINE__);
      mcx_cu_assess(cudaMemset(gfield, 0, sizeof(float) * fieldlen), __FILE__,
                    __LINE__);  // cost about 1 ms
      mcx_cu_assess(cudaMemcpyToSymbol(gcfg, &param, sizeof(MCXParam), 0,
                                       cudaMemcpyHostToDevice),
                    __FILE__, __LINE__);

      tic = StartTimer();
      MCX_FPRINTF(cfg->flog, "generating %d random numbers ... \t", fieldlen);
      fflush(cfg->flog);
      ...
      cudaMemcpy(field, gfield, sizeof(float) * dimxyz * gpu[gpuid].maxgate,
...
#pragma omp barrier
  ...
  mcx_cu_assess(
      ...
      __FILE__, __LINE__);
  mcx_cu_assess(cudaMalloc((void **)&gPdet,
                           sizeof(float) * cfg->maxdetphoton * (detreclen)),
                __FILE__, __LINE__);
  mcx_cu_assess(cudaMalloc((void **)&gdetected, sizeof(uint)), __FILE__,
                __LINE__);
  mcx_cu_assess(
      cudaMalloc((void **)&genergy, sizeof(float) * gpu[gpuid].autothread * 3),
      __FILE__, __LINE__);
  if (cfg->issaveseed) {
    seeddata = (RandType *)malloc(sizeof(RandType) * cfg->maxdetphoton *
                                  RAND_SEED_LEN);
    mcx_cu_assess(
        cudaMalloc((void **)&gseeddata,
                   sizeof(RandType) * cfg->maxdetphoton * RAND_SEED_LEN),
        __FILE__, __LINE__);
  }
  if (cfg->seed == SEED_FROM_FILE) {
    mcx_cu_assess(cudaMalloc((void **)&gPseed,
                             sizeof(float) * cfg->nphoton * RAND_SEED_LEN),
                  __FILE__, __LINE__);
    mcx_cu_assess(cudaMemcpy(gPseed, cfg->replay.seed,
                             sizeof(float) * cfg->nphoton * RAND_SEED_LEN,
                             cudaMemcpyHostToDevice),
                  __FILE__, __LINE__);
    if (cfg->replay.weight) {
      mcx_cu_assess(
          cudaMalloc((void **)&greplayw, sizeof(float) * cfg->nphoton),
          __FILE__, __LINE__);
      mcx_cu_assess(
          cudaMemcpy(greplayw, cfg->replay.weight, sizeof(float) * cfg->nphoton,
                     cudaMemcpyHostToDevice),
          __FILE__, __LINE__);
    }
    if (cfg->replay.tof) {
      mcx_cu_assess(
          ...
          __FILE__, __LINE__);
      mcx_cu_assess(
          cudaMemcpy(greplaytof, cfg->replay.tof, sizeof(float) * cfg->nphoton,
                     cudaMemcpyHostToDevice),
          __FILE__, __LINE__);
    }
  } else
    mcx_cu_assess(
        cudaMalloc((void **)&gPseed,
                   sizeof(uint) * gpu[gpuid].autothread * RAND_SEED_LEN),
        __FILE__, __LINE__);
...
#pragma omp master
  if (cfg->issavedet) {
    MCX_FPRINTF(stderr,
                "WARNING: this MCX binary can not save partial path, please "
                "use mcx_det or mcx_det_cached\n");
    cfg->issavedet = 0;
  }
#pragma omp barrier
#endif
  ...
  if (p0.x < 0.f || p0.y < 0.f || p0.z < 0.f || p0.x >= cfg->dim.x ||
      p0.y >= cfg->dim.y || p0.z >= cfg->dim.z) {
    param.idx1dorig = 0;
    param.mediaidorig = 0;
  } else {
    param.idx1dorig = (int(floorf(p0.z)) * dimlen.y +
                       int(floorf(p0.y)) * dimlen.x + int(floorf(p0.x)));
  ...
  // start wk partition 
  //-------------------------------------------------------------------------//
  if (cfg->wkpartition && (wk_stat == COMPUTE)) {
    //-----------------------------------------------------------------------//
    // prepare parameters 
    //-----------------------------------------------------------------------//
    // gmedia : copy
    ...
    // genergy : copy
    cudaMemcpy(genergy, energy, sizeof(float) * gpu[gpuid].autothread * 3,
               cudaMemcpyHostToDevice);
    // gsrcpattern : copy
    cudaMemcpy(gsrcpattern, cfg->srcpattern,
               ...
               cudaMemcpyHostToDevice);
    // constant memory
    cudaMemcpyToSymbol(gproperty, cfg->prop, cfg->medianum * sizeof(Medium), 0,
                       cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol(gdetpos, cfg->detpos, cfg->detnum * sizeof(float4), 0,
                       cudaMemcpyHostToDevice);

    // allocate shared memory
    sharedbuf = gpu[gpuid].autoblock *
                (sizeof(RandType) * RAND_SEED_LEN + sizeof(MCXdir));
#ifdef USE_CACHEBOX
    if (cfg->sradius > EPS || ABS(cfg->sradius + 1.f) < EPS)
      sharedbuf += sizeof(float) * ((cp1.x - cp0.x + 1) * (cp1.y - cp0.y + 1) *
                                    (cp1.z - cp0.z + 1));
#endif
    if (cfg->issavedet)
...
#ifdef USE_MT_RAND
    sharedbuf +=
        (N + 2) * sizeof(uint);  // MT RNG uses N+2 uint in the shared memory
#endif

    // constant memory
    param.twin0 = cfg->tstart + cfg->tstep * 0;
    param.twin1 = param.twin0 + cfg->tstep * gpu[gpuid].maxgate;
    cudaMemcpyToSymbol(gcfg, &param, sizeof(MCXParam), 0,
                       cudaMemcpyHostToDevice);

    // gfield : memset
    cudaMemset(gfield, 0, sizeof(float) * fieldlen);
    // gseeddata : memset
    if(cfg->issaveseed)
      cudaMemset(gseeddata, 0, sizeof(RandType) * cfg->maxdetphoton * RAND_BUF_LEN);
    // gdetected : memset
    cudaMemset(gdetected, 0, sizeof(float));
    // gPpos : copy
    cudaMemcpy(gPpos, Ppos, sizeof(float4) * gpu[gpuid].autothread,
               cudaMemcpyHostToDevice);
    // gPdir : copy
    cudaMemcpy(gPdir, Pdir, sizeof(float4) * gpu[gpuid].autothread,
               cudaMemcpyHostToDevice);
    // gPlen : copy
    cudaMemcpy(gPlen, Plen, sizeof(float4) * gpu[gpuid].autothread,
               cudaMemcpyHostToDevice);
    // gPseed
    //cudaFree(gPseed);
    //cudaMalloc((void **)&gPseed,
    //           sizeof(uint) * gpu[gpuid].autothread * RAND_SEED_LEN);
    for (int i = 0; i < gpu[gpuid].autothread * RAND_SEED_LEN; i++)
      Pseed[i] = rand();
    cudaMemcpy(gPseed, Pseed,
               sizeof(uint) * gpu[gpuid].autothread * RAND_SEED_LEN,
               cudaMemcpyHostToDevice);
    // use random seed, instead of file
    // ignore greplayw and greplaytof

    //-----------------------------------------------------------------------//
    // 1st sampling point : 1M
    //-----------------------------------------------------------------------//
    tic=StartTimer();

    unsigned int cfg_nphoton = 1e6;
    param.threadphoton = cfg_nphoton / gpu[gpuid].autothread;
    param.oddphotons = cfg_nphoton - param.threadphoton * gpu[gpuid].autothread;

    //MCX_FPRINTF(
    //    cfg->flog,
    //    "\n[wk-1M]GPU=%d threadph=%d oddphotons=%d np=%d nthread=%d maxgate=%d "
    //    "repetition=%d\n",
    //    gpuid + 1, param.threadphoton, param.oddphotons, cfg_nphoton,
    //    gpu[gpuid].autothread, gpu[gpuid].maxgate, cfg->respin);

    tic0 = GetTimeMillis();

    mcx_main_loop <<< mcgrid, mcblock, sharedbuf>>>
      (gmedia, gfield, genergy, gPseed, gPpos, gPdir, gPlen, gPdet, gdetected,
       gsrcpattern, greplayw, greplaytof, gseeddata);

    tic1 = GetTimeMillis();

    // save the 1st sampling point
    wk_b[gpuid] = sampling1[gpuid] = (float)(tic1 - tic0);


    //-----------------------------------------------------------------------//
    // 2nd sampling point : 5M
    //-----------------------------------------------------------------------//
    tic=StartTimer();
...
//    MCX_FPRINTF(
//        cfg->flog,
//        "\n[wk-2M]GPU=%d threadph=%d oddphotons=%d np=%d nthread=%d maxgate=%d "
//        "repetition=%d\n",
//        gpuid + 1, param.threadphoton, param.oddphotons, cfg_nphoton,
//        gpu[gpuid].autothread, gpu[gpuid].maxgate, cfg->respin);
    ...
    // save the 2nd sampling point
    sampling2[gpuid] = (float)(tic1 - tic0);

    // divide by 1M photons
    wk_a[gpuid] = (sampling2[gpuid] - sampling1[gpuid]) / 4.f;

    // avoid a = 0.f
    while(abs(wk_a[gpuid]) < 0.001f) {
      tic0 = GetTimeMillis();
      mcx_main_loop <<< mcgrid, mcblock, sharedbuf>>>
        (gmedia, gfield, genergy, gPseed, gPpos, gPdir, gPlen, gPdet, gdetected,
         gsrcpattern, greplayw, greplaytof, gseeddata);
    ...
    // MCX_FPRINTF(cfg->flog, "=> %f\n", wk_a[gpuid]);
        
#pragma omp barrier 
#pragma omp master
    {
      fo = fopen(".work_config", "a");
      if (fo == NULL) {
        fprintf(stderr, "Fail to create .work_config file.\n");
        exit(1);
      }
      ...
      for(int devid=0; devid<workdev; devid++) {
        // sprintf(buffer, "device %d: a %f\tb %f\n", devid, wk_a[devid], wk_b[devid]);
        // wk_config.append(buffer);
        // fprintf(fo, "device %d: a %f \t b %f\n", devid, wk_a[devid], wk_b[devid]);
        fprintf(fo, "%f %f\n", wk_a[devid], wk_b[devid]);
      }
      fclose(fo);
      //std::cout << wk_config << std::endl;
    }
...
#pragma omp master
    {
      // fixme: use linear programming optimizer, e.g. fminimax in matlab
      // current workload
      float z = (float)(cfg->nphoton) / 1e6;

      // predict the execution time for current workload
      float sum_freq = 0.f;
      for (int devid = 0; devid < workdev; devid++) {
        timeDev[devid] = (z - 1) * wk_a[devid] + wk_b[devid];
        freqDev[devid] = 1 / timeDev[devid];
        sum_freq += freqDev[devid];
      }

      // update the partition data
      for (int devid = 0; devid < workdev; devid++) {
        probDev[devid] = freqDev[devid] / sum_freq;
        printf("device %d : %f\n", devid, probDev[devid]);
      }
    }
#pragma omp barrier 
  ...
  // partition the workload to devices
  if(cfg->wkpartition) {
    gpuphoton = cfg->nphoton * probDev[threadid];
  } else {
    gpuphoton = cfg->nphoton * cfg->workload[threadid] / fullload;
  }
...
#pragma omp master
  {
    ...
    MCX_FPRINTF(cfg->flog,
                "- variant name: [%s] compiled for GPU Capability [%d] with "
                "CUDA [%d]\n",
                "Fermi", MCX_CUDA_ARCH, CUDART_VERSION);
#else
    MCX_FPRINTF(cfg->flog,
                "- code name: [Vanilla MCX] compiled for GPU Capacity [%d] "
                "with CUDA [%d]\n",
...
#else
    MCX_FPRINTF(cfg->flog,
                "- this version CAN NOT save photons at the detectors\n\n");
#endif
    fflush(cfg->flog);
  }
  MCX_FPRINTF(cfg->flog,
              "\nGPU=%d threadph=%d oddphotons=%d np=%d nthread=%d maxgate=%d "
              "repetition=%d\n",
              gpuid + 1, param.threadphoton, param.oddphotons, gpuphoton,
              gpu[gpuid].autothread, gpu[gpuid].maxgate, cfg->respin);
  ...
  /*
     ...
     total
      memory to save all the snapshots, i.e. size(field)*(tend-tstart)/tstep,
     one
      simply sets gpu[gpuid].maxgate to the total gate number; this will run GPU
     kernel
      once. If the required memory is bigger than the video memory, set
     gpu[gpuid].maxgate
      to a number which fits, and the snapshot will be saved with an increment
     of
  ...
  */
  ...
  for (timegate = 0; timegate < totalgates; timegate += gpu[gpuid].maxgate) {
    ...
    MCX_FPRINTF(cfg->flog,
                "lauching MCX simulation for time window [%.2ens %.2ens] ...\n",
    ...
    for (iter = 0; iter < cfg->respin; iter++) {
      ...
      if (cfg->issaveseed)
        mcx_cu_assess(
            cudaMemset(gseeddata, 0,
                       sizeof(RandType) * cfg->maxdetphoton * RAND_BUF_LEN),
            __FILE__, __LINE__);
      mcx_cu_assess(cudaMemset(gdetected, 0, sizeof(float)), __FILE__,
                    __LINE__);
      ...
      tic0 = GetTimeMillis();
      MCX_FPRINTF(cfg->flog, "simulation run#%2d ... \t", iter + 1);
      fflush(cfg->flog);
      mcx_main_loop << <mcgrid, mcblock, sharedbuf>>>
          (gmedia, gfield, genergy, gPseed, gPpos, gPdir, gPlen, gPdet,
           gdetected, gsrcpattern, greplayw, greplaytof, gseeddata);

      mcx_cu_assess(cudaThreadSynchronize(), __FILE__, __LINE__);
      mcx_cu_assess(cudaMemcpy(&detected, gdetected, sizeof(uint),
                               cudaMemcpyDeviceToHost),
                    __FILE__, __LINE__);
      tic1 = GetTimeMillis();
      toc += tic1 - tic0;
      ...
      mcx_cu_assess(
          cudaMemcpy(Plen0, gPlen, sizeof(float4) * gpu[gpuid].autothread,
                     cudaMemcpyDeviceToHost),
          __FILE__, __LINE__);
      for (i = 0; i < gpu[gpuid].autothread; i++)
...
#pragma omp critical
          {
            cfg->exportdetected = (float *)realloc(
                cfg->exportdetected,
                (cfg->detectedcount + detected) * detreclen * sizeof(float));
            ...
            memcpy(cfg->exportdetected + cfg->detectedcount * (detreclen), Pdet,
                   detected * (detreclen) * sizeof(float));
            if (cfg->issaveseed && cfg->seeddata)
      ...
      if (cfg->issave2pt) {
        mcx_cu_assess(cudaMemcpy(field, gfield,
...
#pragma omp master
  {
    if (cfg->isnormalized) {
      ...
      if (cfg->outputtype == otFlux || cfg->outputtype == otFluence) {
        scale = (cfg->energytot - cfg->energyesc) /
                (cfg->energytot * Vvox * cfg->tstep * cfg->energyabs);
        if (cfg->unitinmm != 1.f)
          scale *= cfg->unitinmm; /* Vvox (in mm^3 already) * (Tstep) *
                                     (Eabsorp/U) */

        if (cfg->outputtype == otFluence) scale *= cfg->tstep;
    ...
    if (cfg->issave2pt && cfg->parentid == mpStandalone) {
      MCX_FPRINTF(cfg->flog, "saving data to file ... %d %d\t", fieldlen,
                  gpu[gpuid].maxgate);
    ...
    if (cfg->issavedet && cfg->parentid == mpStandalone &&
      ...
      if (cfg->issaveseed) cfg->his.seedbyte = sizeof(RandType) * RAND_BUF_LEN;
...
#pragma omp barrier
  ...
  cudaMemcpy(Ppos, gPpos, sizeof(float4) * gpu[gpuid].autothread,
             cudaMemcpyDeviceToHost);
  cudaMemcpy(Pdir, gPdir, sizeof(float4) * gpu[gpuid].autothread,
             cudaMemcpyDeviceToHost);
  cudaMemcpy(Plen, gPlen, sizeof(float4) * gpu[gpuid].autothread,
             cudaMemcpyDeviceToHost);
  cudaMemcpy(Pseed, gPseed,
             sizeof(uint) * gpu[gpuid].autothread * RAND_SEED_LEN,
             cudaMemcpyDeviceToHost);
  cudaMemcpy(energy, genergy, sizeof(float) * gpu[gpuid].autothread * 3,
             cudaMemcpyDeviceToHost);
...
#ifdef TEST_RACING
  {
    ...
    for (i = 0; i < fieldlen; i++) hitcount += field[i];
    for (i = 0; i < gpu[gpuid].autothread; i++) totalcount += Pseed[i];

    MCX_FPRINTF(cfg->flog,
                "expected total recording number: %f, got %f, missed %f\n",
...
#pragma omp master
  {
    printnum = gpu[gpuid].autothread < cfg->printnum ? gpu[gpuid].autothread
                                                     : cfg->printnum;
    ...
    // all threads
    MCX_FPRINTF(cfg->flog,
                "simulated %d photons (%d) with %d threads (repeat x%d)\nMCX "
                "simulation speed: %.2f photon/ms\n",
                cfg->nphoton, cfg->nphoton, gpu[gpuid].autothread, cfg->respin,
                (double)cfg->nphoton / cfg->runtime);
    fflush(cfg->flog);
    MCX_FPRINTF(cfg->flog,
